...
2. `leaderboard-controller.js` listens via `addEventListener('input', ...)` and reads `searchInput.value`
3. The controller calls `searchByPlayer(allEntries, value)` — pure data transformation
4. The result passes through `filterByDifficulty()` and `applySorting()` (also pure functions)
5. Finally `renderLeaderboard()` clears the `<tbody id="leaderboardBody">` and appends new `<tr>` rows for each match
6. The DOM updates instantly — no reload, no flicker
